...
void main(void)
{
    ...
    {  
	   ...
	   if(menu == 5){
			...
			if(count>0)
			{
				...
				if(menu == 1){
					...
					if(conversion_state == 0){
						...
						sprintf(toLCDPrint, "%.6f uF", capacitor * 0.000000001);
        		    	LCDprint(toLCDPrint, 2, 1);

						capu[capu_i] = capacitor * 0.000000001;
						capu_i++;
					}
					else{
						printf("Capacitance = %f pf \r\n", capacitor * 0.001);
						sprintf(toLCDPrint, "%.6f pF", capacitor * 0.001);
        		    	LCDprint(toLCDPrint, 2, 1);
					}
				}
				else {
					LCDprint("Impedance     ", 1, 1);
					printf("\nThe impedance of the capacitor is %.4f j, in microOhms\r\n", capacitor * 0.000000001 * f);
					sprintf(toLCDPrint, "%.4fj mOhms", capacitor * 0.000000001 * f);
        		    LCDprint(toLCDPrint, 2, 1);
				}
			}
			else
		...
		printf("\nType 0 to exit, 1 to go to menu, 3 to do capacitance measurements: \r\n");
	   	fflush(stdout); // GCC peculiarities: need to flush stdout to get string out without a '\n'
		fgets(buff, sizeof(buff)-1, stdin);
	...
	printf("\nDeviations\n\r");
    printf("# Capacitor uF ; Capacitor 2 \n\r");
    for(i = 0; i <= 5; i++){
        printf("%d. %f   ; \n\r", i + 1, capu[i]);
